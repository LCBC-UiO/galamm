...
```{r}
ggplot(subset(cognition, domain == 1), 
       aes(x = x, y = y)) + 
  geom_point(size = .5, alpha = .3) + 
  geom_line(aes(group = id), size = .4, alpha = .3) + 
...
$$
\mathbf{y} = \boldsymbol{\beta}_{0} + \boldsymbol{\Lambda} \boldsymbol{\eta} + \boldsymbol{\epsilon}
$$

and the structural model

$$
\boldsymbol{\eta} = \gamma_{1} \mathbf{x} + \gamma_{2}\mathbf{x}^{2} + \boldsymbol{\zeta}.
$$

The reduced form of the model is

$$
\mathbf{y} = \boldsymbol{\beta}_{0} + \boldsymbol{\Lambda} \gamma_{1} \mathbf{x} + \boldsymbol{\Lambda} \gamma_{2} x^{2} + \boldsymbol{\Lambda} \boldsymbol{\zeta}.
$$
...
The diagnostic plot is not perfect, but perhaps acceptable.
