...
NB! The model as set up will as set up is not identified if all $\lambda$ are equal, since the term `s(x, by = weight)` also contains a constant component. This problem can be fixed by using the mixed model representation described in the vignette on maximum likelihood estimation.
