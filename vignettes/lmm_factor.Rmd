...
```{r, eval=FALSE}
judge_plmixed <- PLmixed(response ~ 0 + item + (1 | class)
...
Finally we consider the second example in @rockwoodEstimatingComplexMeasurement2019.
...
kyps.item.model <- galamm(
  formula = response ~ 0 + item + lat.var:time2 + lat.var:time3 + lat.var:time4 + (0 + hs:lat.var | hid) + (0 + ms:lat.var | mid) + (0 + lat.var:as.factor(time) | id),
  data = KYPSitemsim, 
