...
```{r}
plot(
  re$trans.U %*% diag(re$trans.D) %*% c(ranef(mod1)$pseudoGroups$`(Intercept)`, fixef(mod1)),
  coef(mod0$gam)
); abline(0, 1)
```
...
We can also now convert the spline coefficients back to their original parametrization. They are not identical to the coefficients found above because we now have also estimated the factor loadings.

```{r}
ml_final <- mlwrapper(opt$par)

u <- re$trans.U %*% diag(re$trans.D) %*% c(ml_final$u[101:108], opt$par[beta_inds])
...
An approximate covariance matrix for the spline coefficients is given by [@woodConfidenceIntervalsGeneralized2006]

$$\left(\mathbf{X}^{T}\mathbf{W}\mathbf{X} + \lambda \mathbf{S}\right)^{-1} \phi$$

where (for the Gaussian case considered here), $\mathbf{W}$ is diagonal with elements $\phi^{-1}$. The smoothing parameter $\lambda$ is the reciprocal the corresponding variance component. We thus have

```{r}
XR <- t(re$trans.U[1:8, 1:8] %*% (Zt[101:108, ] * (1 / re$trans.D)))
XF <- X

X <- cbind(XR, XF)
solve(t(X) %*% X * ml_final$phi + sm$S[[1]] / opt$par[[2]])
```
