...
We can use this gradient to find the maximum likelihood estimates of the model (technically, the Laplace approximate marginal maximum likelihood estimates).

```{r}
optfun <- function(par, theta_inds, beta_inds, lambda_inds){
  ml <- marginal_likelihood(
    y = dat$y,
    trials = rep(1, length(dat$y)),
    X = X,
...
opt <- optim(par_init, fn = optfun, gr = gradfun, 
             theta_inds = theta_inds, beta_inds = beta_inds, 
