...
system.time({
  opt <- optim(par_init + runif(length(par_init), max = .5), 
               fn = fn, gr = gr, 
             method = "L-BFGS-B", lower = bounds,
             control = list(fnscale = -1, lmm = 20), hessian = TRUE)
})
```
...
We can also compute the Hessian exactly using autodiff, with the argument `final = TRUE`:
...
S_autodiff <- solve(-final_model$hessian)

plot(S, S_autodiff, main = "Finite difference and autodiff Hessian"); abline(0, 1)
```
...
Using `optim()` with L-BFGS-B yields slighlty higher likelihood than `lme4`.
...
```{r}
V <- Diagonal(n = nrow(dat1), x = ml_final$phi) + 
