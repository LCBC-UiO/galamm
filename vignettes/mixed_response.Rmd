...
```r
summary(mod)
#> Rank deficient Hessian matrix. Recomputing without variance components.
#> Warning in sqrt(diag(vcov(object, parm = object$parameters$lambda_inds))): NaNs produced
#> Warning in lambda_tmp_se[!is.na(lambda_tmp_se)] <- sqrt(diag(vcov(object, : number of items to replace is not a
#> multiple of replacement length
#> Rank deficient Hessian matrix. Recomputing without variance components.
#> Warning in sqrt(diag(vcov(object, "beta"))): NaNs produced
#> Warning in cbind(Estimate = fixef(object), `Std. Error` = sqrt(diag(vcov(object, : number of rows of result is not
