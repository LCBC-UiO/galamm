...
#> Error in nobars(formula): could not find function "nobars"
```
...
Looking at the summary output, however, reveals that there is an issue here:


```r
summary(mod)
#> Warning in vcov.galamm(object, parm = object$parameters$lambda_inds): Rank deficient Hessian matrix. Could not
...
The Hessian matrix is not invertible, which means that we cannot compute the variance-covariance matrix. We note that the variance of the latent variable is estimated to be exactly zero, and although zero variance estimates is a well known phenomenon in these types of models (see @hodgesRichlyParameterizedLinear2013 for a book-length treatment), it does not seem reasonable in this case that there is no variation in the true fiber intake in this population of bankers and transport workers.
