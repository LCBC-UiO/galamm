...
[`mgcv::s`](https://rdrr.io/pkg/mgcv/man/s.html). It enables the
...
- factor:
  ...
  Optional character argument specifying the loading variable. Case
  sensitive.

## Value

An object of class `xx.smooth.spec`, where `xx` is a basis identifying
code given by the `bs` argument of `s`. It differs from the smooth
...
[`mgcv::s`](https://rdrr.io/pkg/mgcv/man/s.html) should be consulted for
details on how to properly set up smooth terms. In particular, note that
these terms distinguish between ordered and unordered factor terms in
the `by` variable, which can be provided in `...` and is forwarded to
[`mgcv::s`](https://rdrr.io/pkg/mgcv/man/s.html).
