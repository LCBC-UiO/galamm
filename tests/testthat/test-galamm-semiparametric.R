...
test_that("Basic GAMM with factor structures works", {
  ...
  expect_equal(coef(mod),
               c(item1 = 0.575853007561604, item2 = 0.799928932139754, item3 = 0.178232652144315,
                 `s(x):loadingFx1` = 0.568214146152226))

  expect_equal(vcov(mod),
               structure(c(0.00574546768687868, 1.51191515296913e-15, 3.3681535853856e-16,
                           ...
                           0.00574569029879613), dim = c(4L, 4L)))

  expect_equal(factor_loadings(mod),
               structure(c(1, 1.37647313319922, 0.288954660966359, NA, 0.226964851092118,
                           0.138856542736255), dim = 3:2, dimnames = list(c("lambda1", "lambda2",
                                                                            "lambda3"), c("loading", "SE"))))
})
...
test_that("GAMM with factor structures and random effects works", {
  ...
  expect_equal(coef(mod),
               c(item1 = 1.25074625523314, item2 = 1.75502419468873, item3 = 0.379089137221789,
                 `s(x):loadingFx1` = 0.0203784039785214))

  expect_equal(vcov(mod),
               structure(c(0.00455918825566397, 0.00637611723278559, 0.00136714728242274,
                           ...
                           0.0421377265832167), dim = c(4L, 4L)))

  expect_equal(factor_loadings(mod),
               structure(c(1, 1.40041023113521, 0.300271618584508, NA, 0.00344146863189051,
                           0.00208815232968891), dim = 3:2, dimnames = list(c("lambda1",
