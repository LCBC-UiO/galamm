...
test_that("Mixed response works", {
  ...
  expect_equal(summary(mod)$AICtab,
               c(AIC = 3584.79272575342, BIC = 3622.55702359404, logLik = -4619.20400538187,
                 deviance = 3572.79272575342, df.resid = 3994))
  expect_equal(summary(mod)$Lambda, NULL)

  expect_equal(mod$pearson_residuals[c(4, 8, 11)],
...
test_that("Covariate measurement error model works", {
  ...
  tmp <- summary(mod)
  expect_equal(tmp$Lambda, structure(c(1, 1, -0.133902605671257, NA, NA, 0.0512069460559361
  ), dim = 3:2, dimnames = list(c("1", "2", "3"), c("loading",
                                                    "SE"))))
})
