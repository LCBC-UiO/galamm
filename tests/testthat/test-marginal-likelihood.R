...
ml <- marginal_likelihood(
  ...
  Lambdat = Lambdat,
  beta = c(
    `as.factor(item)1` = 0.645688690888936, `as.factor(item)2` = 0.598327965143937,
...
test_that("ML Gaussian model works", {
  ...
  expect_equal(ml$gradient, c(
    1.31400267378012e-06, -7.46553109109982e-07, -2.36088454505896e-13,
...
test_that("marginal likelihood optimization works", {
  expect_equal(opt$par, c(
    0.634441650291438, 0.577989656676028, 0.645706065689437, 0.598347710112585,
...
test_that("Hessian is correct", {
  expect_equal(S, structure(c(
    0.0357330529544458, 0.0107646140959302, 0.000593180001903187,
    ...
    0.000552379489896846, 0.0252733561893284, 0.0560849371316734
  ), dim = c(
    7L,
    7L
  )))
...
lambda_mapping_Zt2 <- lapply(lambda_mapping_Zt, function(x) {
  if (x == -1L) {
    -1L
  } else if (x == 0L) {
    c(0L, -1L)
